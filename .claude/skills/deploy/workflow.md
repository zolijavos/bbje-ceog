...
## PM2 Standalone Mode (CRITICAL)

The project uses `output: 'standalone'` in next.config.js. PM2 **MUST** run the standalone server, NOT `npm start` / `next start`.

**Correct PM2 setup** (handled by `npm run deploy`):
```bash
pm2 start node --name ceog --cwd /path/to/app -- .next/standalone/server.js
```

**NEVER use**:
- `pm2 start npm --name ceog -- start` ← WRONG, runs `next start` which ignores standalone build
- `pm2 restart ceog` without prior `pm2 delete` ← keeps old config (wrong script/cwd)

**Why this matters**: `next start` in standalone mode serves stale cached pages. The standalone `server.js` serves the freshly built bundle correctly.

**If PM2 shows "errored"**: Check `exec cwd` with `pm2 describe ceog` — it must point to the app directory, not `/root`.

## SSH Rules (CRITICAL)

- **NEVER** use `cd` in SSH commands - use `--prefix` (npm) and `-C` (git)
- **ALWAYS** use the exact app paths from the Environment Map
- Deploy command is `npm run deploy` = build + pm2 delete + pm2 start standalone + pm2 save
