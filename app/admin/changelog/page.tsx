...
const changelogData: ChangelogEntry[] = [
  {
    ...
    changes: [
      ...
      },
      {
        type: 'feature',
        category: 'seating',
        titleEn: 'Seating Plan Import from Excel',
        titleHu: 'Ülésrend Import Excelből',
        descriptionEn: 'Production seating import script that loads the finalized seating plan (24 tables, 233 guests, 76 pairs). Matches guests by email, sets up pair relationships, includes dry-run preview and confirmation prompt.',
        descriptionHu: 'Production ülésrend import script a véglegesített ülésrend betöltéséhez (24 asztal, 233 vendég, 76 pár). Email alapján párosít, beállítja a páros kapcsolatokat, tartalmaz előnézetet és megerősítést.',
      },
      {
        type: 'fix',
        category: 'admin',
        titleEn: 'Dietary Requirements Cleanup',
        titleHu: 'Diétás Igények Takarítás',
        descriptionEn: 'Cleared 35 non-meaningful dietary values (n/a, none, no, pork free, etc.) from guest records while preserving real dietary needs (vegetarian, gluten-free, lactose-free, etc.).',
        descriptionHu: '35 értelmetlen diétás érték törölve (n/a, none, no, pork free, stb.) a vendég rekordokból, a valós diétás igények (vegetáriánus, gluténmentes, laktózmentes, stb.) érintetlenül maradtak.',
      },
      {
        type: 'fix',
        category: 'seating',
        titleEn: 'Seating Occupancy Count Fix',
        titleHu: 'Ülésrend Foglaltsági Számolás Javítás',
        descriptionEn: 'Fixed admin seating page showing wrong occupancy numbers when both main guest and partner have separate table assignments. Each assignment now counts as 1 seat correctly.',
        descriptionHu: 'Javítva az admin ülésrend oldal hibás foglaltsági számai amikor a fővendég és partner külön ültetéssel rendelkezik. Minden hozzárendelés mostantól helyesen 1 széknek számít.',
      },
    ],
  },
  {
    ...
    changes: [
      {
        type: 'feature',
        category: 'display',
        titleEn: 'Live Seating Display',
        titleHu: 'Élő Ülésrend Kijelző',
        descriptionEn: 'Full-screen real-time seating display for event venue screens. Shows background image with guest name overlays per table. Guests update live via SSE when checked in (grey italic → black bold). Check-in counter shows progress.',
        descriptionHu: 'Teljes képernyős valós idejű ülésrend a helyszíni kijelzőkhöz. Háttérkép asztalonkénti vendégnév overlay-ekkel. Vendégek valós időben frissülnek check-in-kor (szürke dőlt → fekete félkövér). Check-in számláló mutatja az előrehaladást.',
      },
      {
        type: 'feature',
        category: 'checkin',
        titleEn: 'Check-in Scanner Redesign — CEO Gala Theme',
        titleHu: 'Check-in Scanner Újratervezés — CEO Gála Arculat',
        descriptionEn: 'Complete visual redesign with dark-blue/gold CEO Gala theme. Shows guest details (table, VIP status, dietary needs, partner) before and after check-in. VIP badge based on VIP Reception flag. PWA-ready mobile scanner.',
        descriptionHu: 'Teljes vizuális átdolgozás sötétkék-arany CEO Gála arculattal. Vendég részletek (asztal, VIP státusz, diéta, partner) megjelennek check-in előtt és után. VIP badge a VIP Reception jelölő alapján. PWA-kompatibilis mobil scanner.',
      },
      {
        type: 'feature',
        category: 'seating',
        titleEn: 'Seating Page Redesign — Search, Collapse & Color Coding',
        titleHu: 'Ülésrend Oldal Újratervezés — Keresés, Összecsukás & Színkódolás',
        descriptionEn: 'Global search (guest name/email + table name), collapsible table cards with auto-collapse (5+ tables), VIP/Standard sections, occupancy filters (All/Available/Full/Empty), table sorting (Name/Free seats), and shared search state between Grid and Floor Plan views.',
        descriptionHu: 'Globális keresés (vendég név/email + asztal név), összecsukható asztalkártyák automatikus összecsukással (5+ asztal), VIP/Standard szekciók, telítettségi szűrők (Összes/Van hely/Tele/Üres), asztal rendezés (Név/Szabad helyek), megosztott keresés Grid és Alaprajz nézet között.',
      },
      {
        type: 'feature',
        category: 'seating',
        titleEn: 'Table Occupancy Color Coding',
        titleHu: 'Asztal Telítettségi Színkódolás',
        descriptionEn: 'Table cards show occupancy status with colored borders: grey (empty), blue (<50%), green (≥50%), red (full). KPI stats bar with dynamic colors. Color guide legend at page bottom.',
        descriptionHu: 'Asztal kártyák telítettségi státusz színes kerettel: szürke (üres), kék (<50%), zöld (≥50%), piros (tele). KPI statisztika sáv dinamikus színekkel. Szín jelölések panel az oldal alján.',
      },
      {
        type: 'feature',
        category: 'seating',
        titleEn: 'Guest Status Dots & VIP Badge',
        titleHu: 'Vendég Státusz Pontok & VIP Badge',
        descriptionEn: 'Guest chips show registration status with colored dots (yellow=invited, blue=registered, green=approved, red=declined). VIP badge shown for guests with VIP Reception flag. Unassigned panel grouped by VIP → Invited → Paying.',
        descriptionHu: 'Vendég kártyákon színes pont a regisztrációs státusz alapján (sárga=meghívott, kék=regisztrált, zöld=jóváhagyott, piros=visszautasított). VIP badge a VIP Reception jelölővel rendelkező vendégeknél. Kiosztásra váró panel csoportosítva: VIP → Meghívott → Fizető.',
      },
      {
        type: 'feature',
        category: 'seating',
        titleEn: 'Inline Table Edit & Auto-Arrange',
        titleHu: 'Helyben Asztal Szerkesztés & Automatikus Elrendezés',
        descriptionEn: 'Edit table name, capacity and type (Standard/VIP/Reserved) directly from the seating grid via popup modal. Auto-arrange button on Floor Plan distributes tables in a grid pattern (VIP front, Standard back).',
        descriptionHu: 'Asztal név, kapacitás és típus (Standard/VIP/Foglalt) szerkesztése közvetlenül az ülésrend Grid nézetből popup ablakkal. Automatikus elrendezés gomb az Alaprajzon rácsba rendezi az asztalokat (VIP elöl, Standard hátul).',
      },
      {
        type: 'feature',
        category: 'seating',
        titleEn: 'Floor Plan Heatmap & Spotlight Search',
        titleHu: 'Alaprajz Heatmap & Spotlight Keresés',
        descriptionEn: 'Floor Plan tables colored by occupancy (grey/green/orange/red heatmap). Search highlights matching tables with pulsating gold glow, dims others. Auto-pan/zoom to single match. Heatmap legend on canvas.',
        descriptionHu: 'Alaprajz asztalok telítettség szerinti színezéssel (szürke/zöld/narancs/piros heatmap). Keresés kiemeli a talált asztalokat pulzáló arany kerettel, a többit elhalványítja. Auto-pan/zoom egyetlen találatra. Heatmap legenda a vásznon.',
      },
      {
        type: 'feature',
        category: 'email',
        titleEn: 'New Email Templates',
        titleHu: 'Új Email Sablonok',
        descriptionEn: 'Added reminder_1month (1-month reminder with partner logos), reminder_2wks (2-week confirmation), invitation_reminder_v2, and sponsor_invitation (PwC Hungary). DB templates take priority over code for live editing.',
        descriptionHu: 'Hozzáadva reminder_1month (1 hónapos emlékeztető partner logókkal), reminder_2wks (2 hetes megerősítés), invitation_reminder_v2, és sponsor_invitation (PwC Hungary). DB sablonok elsőbbséget élveznek az élő szerkesztéshez.',
      },
      {
        type: 'improvement',
        category: 'seating',
        titleEn: 'Grid Tooltip & Floor Plan Tooltip Improvements',
        titleHu: 'Grid Tooltip & Alaprajz Tooltip Fejlesztések',
        descriptionEn: 'Collapsed table cards show guest name tooltip on hover. Floor Plan tooltip: longer display time (800ms), larger scroll area, better mouse reach. Dark mode support throughout.',
        descriptionHu: 'Csukott asztal kártyákon hover tooltip vendégnevekkel. Alaprajz tooltip: hosszabb megjelenítés (800ms), nagyobb scroll terület, jobb egér elérhetőség. Teljes dark mode támogatás.',
      },
      {
        type: 'fix',
        category: 'admin',
        titleEn: 'Guest Edit Validation Fix',
        titleHu: 'Vendég Szerkesztés Validáció Javítás',
        descriptionEn: 'Fixed 400 error when editing guests with cancelled or checked_in status. Added missing enum values to Zod validation schema.',
        descriptionHu: 'Javítva a 400-as hiba vendégek szerkesztésekor cancelled vagy checked_in státusszal. Hiányzó enum értékek hozzáadva a Zod validációs sémához.',
      },
      {
        type: 'fix',
        category: 'admin',
        titleEn: 'Table Type Labels Fixed',
        titleHu: 'Asztal Típus Címkék Javítva',
        descriptionEn: 'Table Management dropdown now correctly shows "VIP" instead of "Meghívott". Separate translation keys for table types vs guest types.',
        descriptionHu: 'Table Management dropdown mostantól helyesen mutatja a "VIP"-t a "Meghívott" helyett. Külön fordítási kulcsok az asztal típusokhoz és vendég típusokhoz.',
      },
      {
        type: 'improvement',
        category: 'testing',
        titleEn: 'E2E Test Suite — 97/97 Pass',
        titleHu: 'E2E Teszt Csomag — 97/97 Sikeres',
        descriptionEn: 'All 97 E2E tests pass. Fixed 20 broken tests from scanner redesign and display feature. Updated selectors, removed deleted elements, fixed parseTableNumber for Hungarian table names.',
        descriptionHu: 'Mind a 97 E2E teszt sikeres. 20 elromlott teszt javítva a scanner átdolgozás és kijelző funkció után. Frissített selectorok, törölt elemek eltávolítva, parseTableNumber javítva magyar asztalnevekhez.',
      },
    ],
  },
  {
    ...
    changes: [
      {
        type: 'feature',
        category: 'email',
        titleEn: 'Two-Week Reminder Email Template',
        titleHu: '2 Hetes Emlékeztető Email Sablon',
        descriptionEn: 'New reminder_2wks email template for 2-week attendance confirmation reminder with partner branding.',
        descriptionHu: 'Új reminder_2wks email sablon 2 hetes részvételi megerősítő emlékeztetőhöz partner brandinggel.',
      },
      {
        type: 'fix',
        category: 'email',
        titleEn: 'Conditional QR Code Attachment',
        titleHu: 'Feltételes QR Kód Csatolás',
        descriptionEn: 'QR code is only attached to emails when the template actually uses the guestQrCode variable, preventing unnecessary attachments.',
        descriptionHu: 'QR kód csak akkor kerül csatolásra, ha a sablon ténylegesen használja a guestQrCode változót, megelőzve a felesleges csatolmányokat.',
      },
      {
        type: 'fix',
        category: 'ui',
        titleEn: 'Email Preview Modal Mobile Fix',
        titleHu: 'Email Előnézet Modal Mobil Javítás',
        descriptionEn: 'Added bottom padding to EmailPreviewModal to prevent mobile footer from overlapping content.',
        descriptionHu: 'Bottom padding hozzáadva az EmailPreviewModal-hoz, hogy a mobil footer ne takarja el a tartalmat.',
      },
    ],
  },
  {
    ...
    changes: [
      {
        type: 'feature',
        category: 'email',
        titleEn: 'VIP Invitation Email Template',
        titleHu: 'VIP Meghívó Email Sablon',
        descriptionEn: 'New vip_invitation template with BBJ logo, 3 signatures, personal notice section, and QR code section. Fully customizable from admin email templates.',
        descriptionHu: 'Új vip_invitation sablon BBJ logóval, 3 aláírással, személyes üzenet szekcióval és QR kód szekcióval. Teljesen testreszabható az admin email sablonokból.',
      },
      {
        type: 'fix',
        category: 'email',
        titleEn: 'QR Code CID Attachment',
        titleHu: 'QR Kód CID Csatolás',
        descriptionEn: 'QR code delivery switched from inline base64 to CID attachment for better email client compatibility. Fixed title duplication in template variables.',
        descriptionHu: 'QR kód küldés átállítva inline base64-ről CID csatolásra a jobb email kliens kompatibilitás érdekében. Cím duplikáció javítva a sablon változókban.',
      },
      {
        type: 'improvement',
        category: 'email',
        titleEn: 'DB-First Template Loading',
        titleHu: 'DB-Elsőbbségű Sablon Betöltés',
        descriptionEn: 'Email system now loads templates from database first with code fallback. Allows live editing of templates without code deployment.',
        descriptionHu: 'Email rendszer mostantól először az adatbázisból tölti be a sablonokat, kód fallback-kel. Lehetővé teszi a sablonok élő szerkesztését kód deploy nélkül.',
      },
      {
        type: 'fix',
        category: 'email',
        titleEn: 'Template Initialization Preserves DB Edits',
        titleHu: 'Sablon Inicializálás Megőrzi a DB Módosításokat',
        descriptionEn: 'Fixed initializeDefaultTemplates to not overwrite database edits on server startup.',
        descriptionHu: 'Javítva az initializeDefaultTemplates, hogy ne írja felül az adatbázis szerkesztéseket szerver indításkor.',
      },
      {
        type: 'feature',
        category: 'admin',
        titleEn: 'Deploy Slash Command',
        titleHu: 'Deploy Slash Command',
        descriptionEn: 'New /deploy slash command for streamlined production deployment directly from Claude Code.',
        descriptionHu: 'Új /deploy slash command az egyszerűsített éles deploy-hoz közvetlenül a Claude Code-ból.',
      },
    ],
  },
  {
    ...
    changes: [
      {
        type: 'feature',
        category: 'email',
        titleEn: 'Invite Reminder V3 Template',
        titleHu: 'Meghívó Emlékeztető V3 Sablon',
        descriptionEn: 'New invite_reminder_v3 email template with Little Minds and Corinthia partner logos in footer. Registered in dropdown, allowed slugs, preview sample data, and i18n.',
        descriptionHu: 'Új invite_reminder_v3 email sablon Little Minds és Corinthia partner logókkal a láblécben. Regisztrálva a dropdown-ban, engedélyezett slug-okban, előnézeti mintaadatokban és i18n-ben.',
      },
      {
        type: 'fix',
        category: 'email',
        titleEn: 'Preview Base URL Fix',
        titleHu: 'Előnézet Base URL Javítás',
        descriptionEn: 'Email template preview now uses APP_URL environment variable instead of hardcoded URL for correct image rendering.',
        descriptionHu: 'Email sablon előnézet mostantól APP_URL környezeti változót használ hardcoded URL helyett a helyes képmegjelenítéshez.',
      },
    ],
  },
  {
    ...
    changes: [
      {
        type: 'improvement',
        category: 'email',
        titleEn: 'Template Upsert on Startup',
        titleHu: 'Sablon Upsert Indításkor',
        descriptionEn: 'initializeDefaultTemplates changed to upsert mode: synchronizes database templates from code on every server startup without overwriting manual edits.',
        descriptionHu: 'initializeDefaultTemplates átállítva upsert módra: szinkronizálja a kód sablonokat az adatbázissal minden szerver indításkor, manuális szerkesztések felülírása nélkül.',
      },
      {
        type: 'fix',
        category: 'email',
        titleEn: 'Email Send Status Protection',
        titleHu: 'Email Küldés Státusz Védelem',
        descriptionEn: 'Fixed email sending to not downgrade registered/approved guest status. Prevents accidental status regression during bulk sends.',
        descriptionHu: 'Javítva az email küldés, hogy ne írja vissza a registered/approved vendég státuszt. Megelőzi a véletlen státusz visszalépést tömeges küldésnél.',
      },
      {
        type: 'improvement',
        category: 'email',
        titleEn: 'Raise Email Send Limit to 500',
        titleHu: 'Email Küldési Limit Emelés 500-ra',
        descriptionEn: 'Bulk email sending limit raised from default to 500 recipients per batch for large guest lists.',
        descriptionHu: 'Tömeges email küldési limit emelve 500 címzettre kötegelt nagy vendéglistákhoz.',
      },
      {
        type: 'improvement',
        category: 'admin',
        titleEn: '100/Page Pagination Option',
        titleHu: '100/Oldal Lapozási Opció',
        descriptionEn: 'Added 100 items per page option to guest list inline pagination for faster browsing of large lists.',
        descriptionHu: 'Hozzáadva 100 elem/oldal opció a vendéglista inline lapozásához a nagy listák gyorsabb áttekintéséhez.',
      },
    ],
  },
  {
    ...
    changes: [
      {
        type: 'feature',
        category: 'email',
        titleEn: 'CBRE Email Template',
        titleHu: 'CBRE Email Sablon',
        descriptionEn: 'Added CBRE-branded email template synced from production. Registered in email selector dropdown.',
        descriptionHu: 'Hozzáadva CBRE arculatú email sablon, szinkronizálva production-ból. Regisztrálva az email kiválasztó dropdown-ban.',
      },
      {
        type: 'feature',
        category: 'email',
        titleEn: 'One-Month Reminder Template with Partner Logos',
        titleHu: '1 Hónapos Emlékeztető Sablon Partner Logókkal',
        descriptionEn: 'New reminder_1month email template with Corinthia and Little Minds partner logos in footer. Equal spacing with fixed-width cells.',
        descriptionHu: 'Új reminder_1month email sablon Corinthia és Little Minds partner logókkal a láblécben. Egyenletes térközök fix szélességű cellákkal.',
      },
      {
        type: 'improvement',
        category: 'admin',
        titleEn: 'i18n PageHeader Translation Keys',
        titleHu: 'i18n PageHeader Fordítási Kulcsok',
        descriptionEn: 'Admin PageHeader component now uses translation keys for title and description. Hungarian translations added for all admin pages.',
        descriptionHu: 'Admin PageHeader komponens mostantól fordítási kulcsokat használ a címhez és leíráshoz. Magyar fordítások hozzáadva minden admin oldalhoz.',
      },
    ],
  },
  {
    ...
    changes: [
      {
        type: 'feature',
        category: 'email',
        titleEn: 'Email Template Selector in Guest List',
        titleHu: 'Email Sablon Választó a Vendéglistában',
        descriptionEn: 'New dropdown in guest list to select email template before sending. Allows choosing from all registered templates including sponsor_invitation.',
        descriptionHu: 'Új dropdown a vendéglistában email sablon kiválasztásához küldés előtt. Választás az összes regisztrált sablonból, beleértve a sponsor_invitation-t.',
      },
      {
        type: 'feature',
        category: 'email',
        titleEn: 'Sponsor Invitation Template (PwC Hungary)',
        titleHu: 'Szponzor Meghívó Sablon (PwC Hungary)',
        descriptionEn: 'New sponsor_invitation email template with 3 signatures including László Radványi, CEO PwC Hungary. Professional corporate invitation layout.',
        descriptionHu: 'Új sponsor_invitation email sablon 3 aláírással, beleértve Radványi László, PwC Hungary CEO. Professzionális vállalati meghívó elrendezés.',
      },
      {
        type: 'improvement',
        category: 'admin',
        titleEn: 'Unified CEOG-4 Codebase',
        titleHu: 'Egységesített CEOG-4 Kódbázis',
        descriptionEn: 'Merged CEOG server and MFLabsDev work into unified CEOG-4 repository. BMad framework updated to v6.0.0.',
        descriptionHu: 'CEOG szerver és MFLabsDev munka összevonva az egységesített CEOG-4 repóba. BMad framework frissítve v6.0.0-ra.',
      },
    ],
  },
  {
    ...
    changes: [
      {
        type: 'improvement',
        category: 'admin',
        titleEn: 'Codebase Cleanup for Testing Phase',
        titleHu: 'Kódbázis Tisztítás Tesztelési Fázishoz',
        descriptionEn: 'Comprehensive codebase cleanup and preparation for the testing and correction phase. Removed dev-only artifacts and stabilized build.',
        descriptionHu: 'Átfogó kódbázis tisztítás és felkészülés a tesztelési és javítási fázisra. Fejlesztői artifact-ok eltávolítva és build stabilizálva.',
      },
    ],
  },
  {
    ...
    changes: [
      {
        type: 'feature',
        category: 'email',
        titleEn: 'Invitation Reminder V2 Template',
        titleHu: 'Meghívó Emlékeztető V2 Sablon',
        descriptionEn: 'New invitation_reminder_v2 email template with improved layout and partner branding support.',
        descriptionHu: 'Új invitation_reminder_v2 email sablon javított elrendezéssel és partner branding támogatással.',
      },
      {
        type: 'improvement',
        category: 'email',
        titleEn: 'Scheduled Emails Improvements',
        titleHu: 'Ütemezett Emailek Fejlesztések',
        descriptionEn: 'Enhanced scheduled emails system with better batch processing and status tracking.',
        descriptionHu: 'Továbbfejlesztett ütemezett email rendszer jobb kötegelt feldolgozással és státusz követéssel.',
      },
      {
        type: 'feature',
        category: 'registration',
        titleEn: 'Partner Change Specification',
        titleHu: 'Partner Módosítás Specifikáció',
        descriptionEn: 'Technical specification for partner change functionality added to planning documents.',
        descriptionHu: 'Partner módosítás funkció technikai specifikációja hozzáadva a tervezési dokumentumokhoz.',
      },
    ],
  },
  {
    ...
    changes: [
      {
        type: 'feature',
        category: 'admin',
        titleEn: 'Admin Rate Limits & Partner Validation',
        titleHu: 'Admin Rate Limit & Partner Validáció',
        descriptionEn: 'Enhanced admin with rate limiting controls, partner email validation, and improved error handling for edge cases.',
        descriptionHu: 'Továbbfejlesztett admin rate limiting kontrollokkal, partner email validációval és javított hibakezeléssel speciális esetekre.',
      },
      {
        type: 'feature',
        category: 'admin',
        titleEn: 'Sortable Guest List',
        titleHu: 'Rendezhető Vendéglista',
        descriptionEn: 'Guest list columns are now sortable by clicking column headers. Sort state persisted during session.',
        descriptionHu: 'Vendéglista oszlopok mostantól rendezhetők az oszlopfejlécekre kattintva. Rendezési állapot megőrizve a session során.',
      },
      {
        type: 'fix',
        category: 'ui',
        titleEn: 'Dark Mode Magic Link Badges',
        titleHu: 'Dark Mode Magic Link Badge-ek',
        descriptionEn: 'Fixed magic link status badges to be properly visible in dark mode with correct contrast.',
        descriptionHu: 'Javítva a magic link státusz badge-ek megjelenítése dark mode-ban megfelelő kontraszttal.',
      },
    ],
  },
  {
    ...
    changes: [
      {
        type: 'feature',
        category: 'email',
        titleEn: 'Outlook-Compatible Email Templates',
        titleHu: 'Outlook-Kompatibilis Email Sablonok',
        descriptionEn: 'Email templates rewritten for full Microsoft Outlook compatibility. Table-based layouts replacing modern CSS for maximum email client support.',
        descriptionHu: 'Email sablonok újraírva teljes Microsoft Outlook kompatibilitással. Tábla-alapú elrendezések a modern CSS helyett maximális email kliens támogatásért.',
      },
      {
        type: 'improvement',
        category: 'email',
        titleEn: 'Remove Database Template Override',
        titleHu: 'Adatbázis Sablon Felülírás Eltávolítása',
        descriptionEn: 'Removed automatic database template override on startup to prevent loss of manually edited templates.',
        descriptionHu: 'Eltávolítva az automatikus adatbázis sablon felülírás indításkor a manuálisan szerkesztett sablonok elvesztésének megakadályozásához.',
      },
    ],
  },
  {
    ...
    changes: [
      {
        type: 'improvement',
        category: 'registration',
        titleEn: 'Remove Seating Preferences from Registration',
        titleHu: 'Ülésrendi Preferenciák Eltávolítása Regisztrációból',
        descriptionEn: 'Seating preferences field removed from registration forms to simplify the guest experience. Position field added to feedback emails instead.',
        descriptionHu: 'Ülésrendi preferenciák mező eltávolítva a regisztrációs űrlapokból az egyszerűbb vendég élmény érdekében. Pozíció mező hozzáadva a visszajelző emailekhez.',
      },
      {
        type: 'fix',
        category: 'ui',
        titleEn: 'Consent Checkbox Dark Background',
        titleHu: 'Hozzájárulás Checkbox Sötét Háttér',
        descriptionEn: 'Fixed consent checkboxes to be readable on dark background. Improved contrast for WCAG compliance.',
        descriptionHu: 'Javítva a hozzájárulás checkboxok olvashatósága sötét háttéren. Javított kontraszt WCAG megfelelőséghez.',
      },
      {
        type: 'fix',
        category: 'admin',
        titleEn: 'User Creation Name Split',
        titleHu: 'Felhasználó Létrehozás Név Szétválasztás',
        descriptionEn: 'Fixed user creation API to properly split name into first_name and last_name fields.',
        descriptionHu: 'Javítva a felhasználó létrehozás API a név helyes szétválasztásához first_name és last_name mezőkre.',
      },
      {
        type: 'fix',
        category: 'admin',
        titleEn: 'CSP Update for Testing Hub',
        titleHu: 'CSP Frissítés a Testing Hub-hoz',
        descriptionEn: 'Added Google Fonts and Alpine.js CDN to Content Security Policy for testing hub compatibility.',
        descriptionHu: 'Google Fonts és Alpine.js CDN hozzáadva a Content Security Policy-hoz a testing hub kompatibilitás érdekében.',
      },
    ],
  },
  {
    ...
    changes: [
      {
        type: 'feature',
        category: 'email',
        titleEn: 'Email Improvements from Client Feedback',
        titleHu: 'Email Fejlesztések Ügyfél Visszajelzés Alapján',
        descriptionEn: 'Multiple email template improvements based on client feedback session. Refined wording, layout, and branding elements.',
        descriptionHu: 'Több email sablon fejlesztés az ügyfél visszajelzés alapján. Finomított szövegezés, elrendezés és arculati elemek.',
      },
      {
        type: 'fix',
        category: 'admin',
        titleEn: 'Partner Display Logic in Guest List',
        titleHu: 'Partner Megjelenítés Logika a Vendéglistában',
        descriptionEn: 'Fixed partner info display: partner guests no longer show their own partner info redundantly. Corrected display logic for paired tickets.',
        descriptionHu: 'Javítva a partner info megjelenítés: partner vendégeknél nem jelenik meg redundánsan a saját partner infójuk. Javított megjelenítési logika páros jegyeknél.',
      },
      {
        type: 'fix',
        category: 'ui',
        titleEn: 'Multiple UI Fixes from Client Feedback',
        titleHu: 'Több UI Javítás Ügyfél Visszajelzés Alapján',
        descriptionEn: 'Removed status badge and VIP labels from registration screens. Various visual corrections based on client review session.',
        descriptionHu: 'Státusz badge és VIP címkék eltávolítva a regisztrációs képernyőkről. Különböző vizuális javítások az ügyfél áttekintés alapján.',
      },
      {
        type: 'improvement',
        category: 'email',
        titleEn: 'QR Email Delay Reduced',
        titleHu: 'QR Email Késleltetés Csökkentve',
        descriptionEn: 'QR ticket email delay reduced from 5 minutes to 3 minutes for faster delivery after registration.',
        descriptionHu: 'QR jegy email késleltetés csökkentve 5 percről 3 percre a regisztráció utáni gyorsabb kézbesítésért.',
      },
    ],
  },
  {
